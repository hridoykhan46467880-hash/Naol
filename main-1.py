...
def extract_otp(body: str) -> str | None:
    """
    ইমেইল বডি থেকে OTP / ভেরিফিকেশন কোড বের করে।
    ৪ থেকে ৮ ডিজিটের সংখ্যা খোঁজে।
    """
    # ওয়ার্ড বাউন্ডারিতে ৪-৮ ডিজিটের কোড খোঁজা
    patterns = [
        r'\b([0-9]{6})\b',   # ৬ ডিজিট (সবচেয়ে সাধারণ OTP)
        r'\b([0-9]{4})\b',   # ৪ ডিজিট
        r'\b([0-9]{8})\b',   # ৮ ডিজিট
        r'\b([0-9]{5})\b',   # ৫ ডিজিট
        r'\b([0-9]{7})\b',   # ৭ ডিজিট
    ...
    for pattern in patterns:
        ...
        if matches:
            return matches[0]  # প্রথম ম্যাচ রিটার্ন
    return None


def clean_body(body: str, limit: int = 800) -> str:
...
def inbox_keyboard() -> InlineKeyboardMarkup:
    """ইনবক্সের জন্য আলাদা কীবোর্ড — রিফ্রেশ বাটন সহ।"""
    buttons = [
        [
            InlineKeyboardButton("🔄 রিফ্রেশ করো", callback_data="inbox"),
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome = (
        ...
        f"🔢 <b>নতুন ফিচার:</b> OTP কোড অটো-ডিটেক্ট!\n\n"
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── GET MAIL ──────────────────────────────
        ...
        text = (
            ...
            f"🔢 OTP কোড আসলে বট <b>অটোমেটিক হাইলাইট</b> করে দেবে!\n"
            f"🔄 নতুন মেইল চাইলে আবার <b>GET MAIL</b> চাপো\n\n"
            f"⚠️ <i>নোট: টেম্প মেইল সাময়িক, গুরুত্বপূর্ণ কাজে ব্যবহার করো না।</i>"
        )
        await query.edit_message_text(text, parse_mode="HTML", reply_markup=inbox_keyboard())
    ...
    # ── INBOX ─────────────────────────────────
        ...
        if not messages:
            text = (
                ...
                f"🔄 আবার চেক করতে নিচের <b>রিফ্রেশ</b> বাটন চাপো।"
            )
            await query.edit_message_text(text, parse_mode="HTML", reply_markup=inbox_keyboard())
            return

        # ── মেইল লিস্ট দেখানো (সর্বোচ্চ ৫টি) ──
        lines = [
        ...
        # ── সর্বশেষ মেইলের বডি + OTP ডিটেক্ট ──
        ...
        if detail:
            ...
            # ── OTP পেলে আলাদাভাবে হাইলাইট ──
            if otp:
                lines.append(
                    f"\n\n🎯 ━━━━━━━━━━━━━━━━━━━━ 🎯\n"
                    f"🔢 <b>OTP / ভেরিফিকেশন কোড:</b>\n\n"
                    f"<code>  {otp}  </code>\n\n"
                    f"👆 উপরের কোডটা কপি করো!\n"
                    f"🎯 ━━━━━━━━━━━━━━━━━━━━ 🎯"
                )
            else:
                lines.append(
                    f"\n\n💡 <i>কোনো OTP কোড ডিটেক্ট হয়নি।\n"
                    f"উপরের বডি থেকে কোড নিজে খুঁজো।</i>"
                )

        lines.append("\n\n🔄 নতুন মেইলের জন্য রিফ্রেশ চাপো।")

        await query.edit_message_text(
            "".join(lines), parse_mode="HTML", reply_markup=inbox_keyboard()
        )
